"""
Generate full evaluation report for the RotatE + XGBoost link prediction decoder.

Same evaluation pipeline as Node2Vec but using RotatE embeddings (256-dim).
Produces:
...
All outputs saved to ml/data/rotate/results/.
"""
...
from link_prediction_rotate import (
...
RESULTS_DIR = Path(__file__).resolve().parent / "data" / "rotate" / "results"
MODELS_DIR = Path(__file__).resolve().parent / "data" / "rotate" / "models"
...
def save_classification_report(y_true, y_pred, y_prob):
    ...
    with open(RESULTS_DIR / "classification_report.txt", "w") as f:
        f.write("RotatE + XGBoost Link Prediction — Test Set Classification Report\n")
        f.write("=" * 60 + "\n\n")
        f.write(report_text)
...
def save_confusion_matrix(y_true, y_pred):
    ...
    ax.set_title("RotatE + XGBoost Link Prediction — Confusion Matrix (Test Set)")
    fig.tight_layout()
...
def save_roc_curve(y_true, y_prob):
    ...
    ax.plot(fpr, tpr, color="#16a34a", lw=2,
            label=f"RotatE + XGBoost (AUROC = {roc_auc:.4f})")
    ax.plot([0, 1], [0, 1], color="gray", lw=1, linestyle="--", label="Random")
    ax.set_xlim([-0.02, 1.02])
    ax.set_ylim([-0.02, 1.02])
    ax.set_xlabel("False Positive Rate", fontsize=12)
    ax.set_ylabel("True Positive Rate", fontsize=12)
    ax.set_title("ROC Curve — RotatE + XGBoost Drug–Disease Link Prediction", fontsize=13)
...
def save_pr_curve(y_true, y_prob):
    ...
    ax.plot(recall, precision, color="#dc2626", lw=2,
            label=f"RotatE + XGBoost (AUPRC = {ap:.4f})")
    ...
    ax.set_title("Precision-Recall Curve — RotatE + XGBoost Drug–Disease Link Prediction",
                 fontsize=13)
...
def save_feature_importance(model, emb_dim, top_n=20):
    ...
    colors = ["#16a34a" if not n.startswith(("shared", "jaccard", "adamic", "log_", "cosine", "l2"))
              else "#dc2626" for n in top_names]
    bars = ax.barh(range(top_n), top_vals, color=colors)
    ax.set_yticks(range(top_n))
    ax.set_yticklabels(top_names, fontsize=10)
    ax.set_xlabel("Feature Importance (gain)", fontsize=12)
    ax.set_title(f"Top {top_n} Feature Importances — RotatE + XGBoost Decoder", fontsize=13)

    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor="#16a34a", label="Embedding features"),
...
def main():
    print("Loading RotatE embeddings and metadata...")
    ...
    print("\nTraining XGBoost (same hyperparams as Node2Vec evaluation)...")
    ...
    print("\n" + "=" * 60)
    print("GENERATING EVALUATION REPORT (RotatE)")
    print("=" * 60)
