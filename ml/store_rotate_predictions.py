"""Store RotatE predictions in Memgraph alongside Node2Vec predictions."""
...
PREDICTIONS_PATH = DATA_DIR / "rotate" / "predictions.tsv"
...
def main():
    ...
    print(f"Loaded {len(predictions)} RotatE predictions")

    driver = GraphDatabase.driver(MEMGRAPH_URI, auth=(MEMGRAPH_USER, MEMGRAPH_PASS))
    try:
        with driver.session() as s:
            r = s.run("MATCH ()-[r:predictedTreatsDisease {source: 'RotatE_LinkPrediction'}]->() DELETE r RETURN count(r)")
            deleted = r.single()[0]
            if deleted:
                print(f"Cleared {deleted} existing RotatE predictions")
        ...
        print(f"Stored {total} RotatE predictedTreatsDisease edges in Memgraph")
