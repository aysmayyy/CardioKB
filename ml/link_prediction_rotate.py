...
def main():
    ...
    log.info("=" * 60)
    log.info(f"{'Decoder':<12} {'AUROC':>8} {'AUPRC':>8}")
    log.info("-" * 30)
    for r in results:
        log.info(f"{r['name']:<12} {r['test_auroc']:>8.4f} {r['test_auprc']:>8.4f}")

    best = max(results, key=lambda r: r["test_auroc"])
    log.info(f"\nBest RotatE decoder: {best['name']} (Test AUROC={best['test_auroc']:.4f})")

    import pickle
    models_dir = ROTATE_DIR / "models"
    ...
    import gzip
    archive_path = ROTATE_DIR / "all_predictions_above_threshold.tsv.gz"
