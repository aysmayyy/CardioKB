"""
Link prediction for drug repurposing using RotatE embeddings.

Same methodology as Node2Vec link prediction for fair comparison:
  - RotatE trained on train edges only (80%)
...
def load_embeddings():
    """Load RotatE entity embeddings (real+imag concatenated)."""
    path = ROTATE_DIR / "rotate_embeddings.npz"
    data = np.load(path)
    node_ids = data["node_ids"]
    embeddings = data["embeddings"]
    emb_map = {int(nid): embeddings[i] for i, nid in enumerate(node_ids)}
    log.info(f"Loaded RotatE embeddings: {embeddings.shape} from {path.name}")
    return emb_map
...
def save_predictions_tsv(predictions, meta):
    out_path = ROTATE_DIR / "predictions.tsv"
...
def save_evaluation_report(results, n_drugs, n_diseases, n_train_pos,
                           n_val_pos, n_test_pos, best_name, node2vec_baseline):
    report = {
        "embedding_method": "RotatE",
        "methodology": "80/10/10 stratified split, RotatE on train only, 1:1 neg ratio",
        ...
        "decoders": {},
        "node2vec_baseline": node2vec_baseline,
    }
    for r in results:
    ...
    out_path = ROTATE_DIR / "evaluation_report.json"
...
def main():
    log.info("=" * 60)
    log.info("CardioKB Link Prediction — RotatE Embeddings")
    log.info("=" * 60)
    ...
    log.info("\nSampling negatives (1:1 ratio, same seeds as Node2Vec)...")
    ...
    # Node2Vec baseline for comparison
    node2vec_baseline = {
    ...
    log.info("\n" + "=" * 60)
    log.info("COMPARISON: RotatE vs Node2Vec")
    log.info("=" * 60)
    log.info(f"{'Decoder':<12} {'RotatE AUROC':>13} {'RotatE AUPRC':>13} "
             f"{'N2V AUROC':>10} {'N2V AUPRC':>10} {'Delta AUROC':>12}")
    log.info("-" * 75)
    for r in results:
        n2v = node2vec_baseline.get(r["name"], {})
        n2v_auroc = n2v.get("test_auroc", 0)
        delta = r["test_auroc"] - n2v_auroc
        sign = "+" if delta >= 0 else ""
        log.info(f"{r['name']:<12} {r['test_auroc']:>13.4f} {r['test_auprc']:>13.4f} "
                 f"{n2v_auroc:>10.4f} {n2v.get('test_auprc', 0):>10.4f} "
                 f"{sign}{delta:>11.4f}")

    best = max(results, key=lambda r: r["test_auroc"])
    log.info(f"\nBest RotatE decoder: {best['name']} (Test AUROC={best['test_auroc']:.4f})")
    ...
    if predictions:
        log.info(f"\nTop 30 predicted drug repurposing candidates (RotatE + {best['name']}):")
