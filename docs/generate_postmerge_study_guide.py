"""Generate CardioKB Post-Merge Study Guide as a formatted Word document.

Post-merge edition (July 11, 2026): Updated with live Memgraph stats after
duplicate Drug node merge (entity resolution), retrained ML models, and
stale-memgraph-ID bug fix.
"""
...
toc_items = [
    ...
    ('6.10 Post-Merge Data Quality Improvements', 1),
    ('6.11 Known Limitations', 1),
    ('6.12 Future ML Directions', 1),
...
bullet('453,037 nodes across 17 distinct node types (26,794 Drug nodes after entity resolution merge)')
bullet('5,461,783 relationships across 28 relationship types')
bullet('23 data sources + 2 ML prediction sources (CompGCN: 6,607 edges, RotatE: 7,828 edges)')
...
bullet('An ML-based drug repurposing pipeline that uses knowledge graph embeddings (CompGCN, RotatE) to predict new drug-disease treatment relationships with AUROC up to 0.9865, producing 14,435 predicted edges stored in the graph')
...
add_table(
    ...
    [
        ...
        ['Integrated ML', '14,435 predicted drug-disease edges stored in graph with confidence scores', 'ML predictions separate from graph, not queryable inline'],
...
doc.add_paragraph(
    'Contribution: 1,449 drugTreatsDisease edges + 5,714 drugTreatsPhenotype edges (post-merge counts). '
...
doc.add_paragraph(
    'The current CardioKB graph (post-merge, July 11, 2026) contains 453,037 nodes across 17 distinct node types '
    'and 5,461,783 relationships across 28 relationship types. There are 23 data sources '
    '(each tagged with a source property), plus 2 ML prediction sources (14,435 predicted edges total). '
...
add_table(
    ...
    [
        ...
        ['Drug', '26,794', 'DrugBank + CTD (deduplicated)', 'Any chemical compound with pharmacological activity. Includes FDA-approved drugs, experimental compounds, and nutraceuticals. Identified by DrugBank ID. Deduplicated from 32,849 by xrefDrugBank entity resolution.'],
...
add_table(
    ...
    [
        ...
        ['chemicalIncreasesExpression', 'Drug→Gene', 'CTD', '343,783', '—', 'This chemical increases the mRNA/protein expression of this gene'],
        ['chemicalDecreasesExpression', 'Drug→Gene', 'CTD', '328,708', '—', 'This chemical decreases the mRNA/protein expression of this gene'],
        ...
        ['compoundCausesSideEffect', 'Drug→SideEffect', 'SIDER', '67,646', '—', 'This drug is known to cause this adverse reaction'],
        ...
        ['compoundInPharmClass', 'Drug→PharmClass', 'DrugCentral', '24,752', '—', 'This drug belongs to this FDA pharmacologic class'],
        ...
        ['drugTreatsPhenotype', 'Drug→Phenotype', 'DrugBank_Indications', '5,714', '—', 'This drug treats this phenotype/condition (text-mined from DrugBank indication fields)'],
        ['drugTreatsDisease', 'Drug→Disease', 'CTD+CT+DC+DBI', '4,852', '—', 'This drug is used to treat this disease (curated + clinical trial + FDA + text-mined evidence)'],
        ['TESTS_INTERVENTION', 'Trial→Drug', 'ClinicalTrials.gov', '3,178', '—', 'This clinical trial tests this drug as an intervention'],
        ['diseaseIsSubtypeOf', 'Disease→Disease', 'Disease Ontology', '2,581', '—', 'This disease is a subtype of this broader disease category'],
        ['predictedTreatsDisease', 'Drug→Disease', 'ML Predictions', '14,435', 'confidence', 'ML model predicts this drug might treat this disease (NOT clinically validated). CompGCN: 6,607, RotatE: 7,828.'],
...
bullet('No license restrictions: Memgraph\'s open-source edition has no node/relationship count limits, whereas Neo4j Community Edition has constraints that would affect a 453K-node graph.')
...
doc.add_paragraph(
    ...
    'entire CardioKB graph (453K nodes, 5.5M edges, 23 sources) can be rebuilt from scratch in approximately '
...
bullet('Node2Vec needed SparseOTF mode on HPC to avoid out-of-memory errors on a 453K-node graph.')
doc.add_paragraph(
...
add_table(
    ...
    [
        ['CTD', '3,099', 'Curated chemical-disease therapeutic relationships from scientific literature', 'Literature-curated, high confidence.'],
        ['DrugBank_Indications', '1,449', 'Text-mined from DrugBank XML indication free-text fields', 'Whole-word disease name matching against existing Disease nodes. Count reduced post-merge due to duplicate Drug node consolidation.'],
        ...
        ['Total (deduplicated)', '4,852', 'Combined, deduplicated by (Drug, Disease) pair after entity resolution merge', '4,726 training edges used as ML training labels.'],
    ],
...
doc.add_paragraph(
    ...
    'against Phenotype node names, creating 5,714 drugTreatsPhenotype edges (Drug→Phenotype, count reduced '
    'post-merge due to duplicate Drug node consolidation). A blocklist filters out HPO modifier terms (Acute, Chronic, Severe, etc.) that would '
...
bold_para('What happened: ', 'Node2Vec requires building a transition probability matrix for random walks. On a 453K-node graph with 5.4M edges, the default approach exhausted available RAM.')
bold_para('SparseOTF vs PreComp vs DenseOTF: ', 'PecanPy (the Node2Vec implementation we used) offers three modes. PreComp pre-computes all transition probabilities — fast walks but requires storing the full matrix in memory (impossible for our graph). DenseOTF computes probabilities on-the-fly using dense matrices — lower memory but still too much for 453K nodes. SparseOTF computes on-the-fly using sparse matrices — the only mode that fits in memory for our graph size. We used SparseOTF mode, which completed successfully on HPC.')
...
bold_para('80/10/10 stratified split: ', '"Stratified" means all edge types are split proportionally. This ensures the train set reflects the full graph topology, not just a biased sample. Split seed = 42 for reproducibility. For drugTreatsDisease specifically: 4,726 train / 485 validation / 484 test. All models retrained on the post-merge graph with zero cross-split leakage confirmed.')
...
bold_para('Therapeutic filter: ', 'Not all 26,794 Drug nodes are therapeutically relevant. Many are metabolites, food compounds, or experimental chemicals with no treatment signal. We filter to drugs that have at least one "therapeutic signal" edge (drugTreatsDisease, drugBindsGene, chemicalIncreasesExpression, etc.). This reduces 26,794 to 10,310 therapeutic drugs. Without this filter, the prediction space would be dominated by irrelevant chemicals.')
...
bold_para('Results: ', 'Best AUROC 0.9504 with XGBoost decoder. The OOM issues during training required using PecanPy\'s SparseOTF mode instead of PreComp (which pre-computes all transition probabilities and exceeds memory on a 453K-node graph).')
...
bold_para('Native MRR: ', '0.1119 (PyKEEN filtered ranking). This is modest because ranking evaluates ALL 453K entities as candidates — the model must rank the correct tail entity above 453,036 others. The XGBoost decoder on the restricted Drug × Disease candidate space is the operationally relevant metric.')
...
bold_para('Results (post-merge): ', 'AUROC 0.9828, AUPRC 0.9812, Hits@10 = 39.3%, Hits@50 = 72.1%, Hits@100 = 84.1%, Hits@200 = 92.8%, MRR = 0.1890, Median rank = 17.5 with XGBoost decoder. RotatE scores 7,828 predictions above the storage threshold across 1,165 drugs and 142 diseases.')
...
bold_para('Results vs RotatE (post-merge): ', 'CompGCN + XGBoost achieves AUROC 0.9865, improving over RotatE by +0.0037. Full ranking metrics: Hits@10 = 36.6%, Hits@50 = 68.0%, Hits@100 = 82.6%, Hits@200 = 93.8%, MRR = 0.2141, Median rank = 22. CompGCN has higher MRR (0.2141 vs 0.1890) but higher median rank (22 vs 17.5) — CompGCN better at reciprocal ranking overall, RotatE slightly tighter at top-of-list. CompGCN scores 6,607 predictions above the storage threshold across 1,038 drugs and 37 diseases; RotatE is more diverse with 142 diseases.')
...
add_table(
    ...
    [
        ['RotatE', '256', 'Cosine', '0.7807', '0.7569', '1.7%', '9.3%', '13.4%', '27.3%', '0.0095', '461'],
        ['RotatE', '256', 'XGBoost', '0.9828', '0.9812', '39.3%', '72.1%', '84.1%', '92.8%', '0.1890', '17.5'],
        ['RotatE', '256', 'MLP', '0.9810', '0.9786', '41.1%', '71.9%', '83.5%', '91.9%', '0.1898', '16'],
        ['CompGCN', '128', 'Cosine', '0.3100', '0.3810', '0.2%', '0.2%', '0.4%', '0.6%', '0.0027', '2,230'],
        ['CompGCN', '128', 'XGBoost', '0.9865', '0.9854', '36.6%', '68.0%', '82.6%', '93.8%', '0.2141', '22'],
        ['CompGCN', '128', 'MLP', '0.9838', '0.9775', '29.3%', '65.5%', '81.4%', '93.2%', '0.1168', '27.5'],
    ],
...
bold_para('Predictions stored in Memgraph: ', 'Predictions are stored as predictedTreatsDisease edges with properties: confidence (XGBoost probability, float 0-1) and source ("RotatE_LinkPrediction" or "CompGCN_LinkPrediction"). CompGCN: 6,607 edges (1,038 drugs × 37 diseases, confidence 0.989–0.991). RotatE: 7,828 edges (1,165 drugs × 142 diseases, confidence 0.993–0.997). Total: 14,435 prediction edges. Displayed in the web UI as orange dashed edges with "not clinically validated" warning.')
...
doc.add_paragraph(
    ...
    'relationship types, and (3) dramatically faster training (~7 min vs ~3.4 hrs). RotatE\'s 7,828 '
    'predictions remain stored in the graph and are fully queryable via the Cypher editor in the Query tab. '
    'In the Explore tab, only CompGCN predictions (6,607 edges) appear as orange dashed edges when the '
    '"Show ML Predictions" toggle is enabled.'
)
...
doc.add_paragraph(
    ...
    'does NOT mean a 99% chance the drug works. CompGCN predictions span 0.989–0.991 and RotatE spans '
    '0.993–0.997, both narrow bands indicating uniformly high model confidence across the ranked lists. '
    'These scores are useful for prioritizing candidates but should never be interpreted as clinical '
    'efficacy estimates. All predicted edges in the UI carry a "not clinically validated" warning.'
)

doc.add_heading('Prediction Coverage Analysis (Post-Merge)', level=3)
doc.add_paragraph(
    'CompGCN: 6,607 stored predictions from 1,038 unique drugs across 37 unique diseases. '
    'RotatE: 7,828 stored predictions from 1,165 unique drugs across 142 unique diseases. '
    'Distribution is concentrated: top 6 diseases (heart disease, coronary artery disease, hypertension, '
    'atherosclerosis, congestive heart failure, myocardial infarction) account for a disproportionate share '
    'of CompGCN predictions. RotatE is more diverse with 142 target diseases vs CompGCN\'s 37. '
    'The post-merge entity resolution eliminated the cross-source Drug node duplication that previously '
    'allowed ~1.5% spurious predictions (known treatments leaking via duplicate nodes) and ~8.7% wasted '
    'slots (same drug-disease pair predicted from both copies). Total: 14,435 predicted edges, all verified '
    'as Drug→Disease with zero wrong-label contamination.'
)

doc.add_heading('6.10 Post-Merge Data Quality Improvements', level=2)
doc.add_paragraph(
    'Three methodology fixes were applied after the initial graph build, resulting in a cleaner graph '
    'and more reliable ML predictions. All ML models were retrained on the corrected graph with zero '
    'cross-split leakage confirmed.'
)

bold_para('ClinicalTrials.gov Inference Fix. ',
    'The original ClinicalTrials.gov parser inferred drugTreatsDisease edges from any Phase 3/4 trial '
    'linking a drug intervention to a disease condition, yielding 868 edges. An audit revealed that many '
    'were spurious: trials with non-treatment primary purposes (e.g., Prevention, Diagnostic), drugs serving '
    'as comparators rather than experimental interventions, and diseases matching secondary rather than '
    'primary conditions. Four filters were applied: (1) primaryPurpose==TREATMENT, (2) drug must be in an '
    'EXPERIMENTAL arm, (3) disease must match the first-listed condition, (4) edges carry a trialCount '
    'property. This reduced ClinicalTrials.gov drugTreatsDisease edges from 868 to 147 (83.1% reduction).')

bold_para('Duplicate Drug Node Merge (Entity Resolution). ',
    'The BaseAgent pipeline created duplicate Drug nodes when multiple sources (DrugBank, CTD, ClinPGx, '
    'DrugCentral) loaded the same compound under different internal drugId values but shared the same '
    'xrefDrugBank canonical identifier. A post-hoc entity resolution step (scripts/merge_duplicate_drugs.py) '
    'identified 5,611 duplicate groups (2x-8x duplication, including salt/ester forms), removed 6,055 '
    'duplicate nodes, transferred 474,641 edges to survivor nodes, and deduplicated 9,094 redundant edges. '
    'Drug nodes went from 32,849 to 26,794. This eliminated the cross-source Drug node duplication that '
    'previously allowed ~1.5% spurious predictions (known treatments leaking via duplicate nodes) and ~8.7% '
    'wasted prediction slots (same drug-disease pair predicted from both copies).')

bold_para('Stale Memgraph-ID Bug Fix. ',
    ...
    'causing predicted edges to land on wrong node types (Gene, PharmacologicClass, SideEffect) that '
    'had inherited the recycled IDs. This was detected when the web UI showed lab reagents and food '
    'chemicals (e.g., 2-Hydroxyestradiol, Tryptamine, DMSO, pyrachlostrobin) as predicted treatments '
    'for heart disease. The fix rewrote store_predictions.py to match Drug/Disease nodes by name from '
    'the live graph instead of stale file-based IDs, making it permanently robust to ID recycling. '
    'All 14,435 predicted edges were verified as Drug -> Disease with zero wrong-label contamination.')

doc.add_heading('6.11 Known Limitations', level=2)

bullet('Confidence scores reflect ranking, not calibrated probability. The XGBoost decoder outputs values '
    '0-1 representing how closely a drug-disease pair\'s embedding geometry matches known treatment '
    'relationships. CompGCN predictions span 0.989-0.991, RotatE spans 0.993-0.997 - narrow bands '
    'indicating uniform model confidence. These are not clinical efficacy estimates.')

bullet('Pharmacologic-class parent nodes in predictions. A subset of top predictions involve '
    'PharmacologicClass parent nodes (e.g., "ACE Inhibitors", "HMG-CoA Reductase Inhibitors") rather than '
    'specific Drug compounds. These class-level predictions are structurally valid but less actionable for '
    'drug repurposing screening.')

bullet('Hub-driven prediction concentration. CompGCN predictions concentrate on high-degree Disease hub '
    'nodes: the top 6 diseases (heart disease, coronary artery disease, hypertension, atherosclerosis, '
    'congestive heart failure, myocardial infarction) account for a disproportionate share. Rarer CVD '
    'conditions receive fewer or no predictions.')

bullet('Disease-specific embedding dominance (CompGCN). Pulmonary embolism has 6,859 incoming edges but '
    'received only 1 CompGCN prediction (rank 7,861 of 10,000), because 92.9% of its edges are '
    'geneAssociatesWithDisease (6,372) versus only 46 drugTreatsDisease. CompGCN\'s neighborhood-aggregation '
    'mechanism causes PE\'s embedding to be dominated by gene-association signal rather than drug-treatment '
    'signal, suppressing confident predictions even though the disease is well-represented overall. RotatE, '
    'which learns independent per-entity embeddings rather than aggregating neighbor information, was less '
    'susceptible to this effect and produced 75 predictions for PE.')

bullet('Positive case study (Dabigatran -> PE). Among RotatE\'s PE predictions, Dabigatran (the active '
    'thrombin inhibitor) was predicted to treat PE despite having no curated drugTreatsDisease edge for PE '
    '- only a CTD edge to stroke. The prodrug form (Dabigatran etexilate, a separate Drug node) does have '
    'a curated PE indication from DrugBank_Indications. The model bridged the active-metabolite/prodrug '
    'entity gap, illustrating both the model\'s ability to surface pharmacologically valid connections and '
    'the role entity resolution plays in prediction quality.')

bullet('Limited graph connectivity drugs. Some predicted drugs have connectivity limited to a '
...
doc.add_paragraph(
    'The "Show ML Predictions" toggle checkbox controls visibility of 14,435 predicted drug-disease edges. '
    'When enabled: orange dashed edges appear connecting drugs to diseases. CompGCN is the primary model '
    'shown in the Explore tab (6,607 edges); RotatE predictions (7,828 edges) are fully queryable via the Cypher editor in the Query tab. '
...
bullet('ML confidence: 100% coverage on predictedTreatsDisease edges (14,435 edges) — no gap')
...
doc.add_paragraph(
    'The post-merge CompGCN results (AUROC 0.9865, best overall) demonstrate that GNN-based approaches outperform '
    'translational (RotatE) embedding methods on this graph. The explainability '
...
doc.add_paragraph(
    ...
    'This added drugTreatsDisease edges and created a new drugTreatsPhenotype '
    'relationship type (5,714 edges post-merge). The phenotype edges address '
...
# ── Save ──
output_path = os.path.join(os.path.dirname(__file__), 'CardioKB_PostMerge_Study_Guide_July11.docx')
