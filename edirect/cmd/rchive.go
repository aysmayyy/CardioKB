...
func main() {
	...
	// flags for separate indexing and inversion steps
	entrezIndex := false
	entrezInvert := false
	...
	for {
		...
		// replacement functions allow script to control incremental indexing and inversion
		case "-entrezIndex":
			entrezIndex = true
		case "-entrezInvert":
			entrezInvert = true
		...
		// -countt tests all term counts for the file containing the specific term (undocumented)
		case "-countt":
			tlst = true
			fallthrough
	...
	*/
	...
	// NEW -entrezIndex function FOLLOWED BY -transform meshtree.txt AND -e2index

	if entrezIndex && len(args) > 0 && args[0] == "-e2index" {
		...
		if len(args) < 1 {
			eutils.DisplayError("-entrezIndex argument generation failure")
			os.Exit(1)
		}
		...
		if cmds == nil {
			eutils.DisplayError("Problem parsing -e2index arguments after -entrezIndex")
			os.Exit(1)
		}
		...
		eiq := eutils.EntrezIndex(recname, callConsumers, in)
		if eiq == nil {
			eutils.DisplayError("Unable to create -entrezIndex channel")
			os.Exit(1)
		}
	...
	if entrezInvert {
		...
		iifq := eutils.EntrezInvert(in)

		if iifq == nil {
			eutils.DisplayError("Unable to create -entrezInvert channel")
			os.Exit(1)
		}
	...
	if trms != "" {
		...
		// deStop should match value used in building the indices
		recordCount = eutils.ProcessCount(db, trms, plrl, psns, tlst, deStop)
	...
	if nvrt {
		...
		colq := eutils.StringToChan(str, chanDepth)
