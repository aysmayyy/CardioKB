	...
	if len(args) == 8 && (args[2] == "-split-by-id") && args[4] == "-prefix" && args[6] == "-suffix" {
		...
		eutils.PartitionXML(topPattern, star, false, rdr,
			func(str string) {
				...
				// id = strings.TrimPrefix(id, prefix) // was old case-sensitive code
				// case-insensitive version handles old PMC and new pmc prefix for article-id pub-id-type="pmcid"
