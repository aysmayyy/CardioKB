...
func main() {
	...
	*/
	...
	if args[0] == "-stream" {
		...
		if len(args) > 1 {
			if args[1] == "-pattern" {
				// skip past -stream if followed by -pattern
				args = args[1:]
			}
		}
		if len(args) < 2 {
			eutils.DisplayError("Pattern missing after -stream command")
			os.Exit(1)
		}
		pat := args[1]
	...
	if args[0] == "-shuffle" {
		...
		if len(args) > 1 {
			if args[1] == "-pattern" {
				// skip past -shuffle if followed by -pattern
				args = args[1:]
			}
		}
		if len(args) < 2 {
			eutils.DisplayError("Pattern missing after -shuffle command")
			os.Exit(1)
		}
		pat := args[1]
				...
				if str != "" && str != "*" {
					...
					if insdx {
						...
						if after, ok := strings.CutPrefix(str, "#"); ok {
							str = after
							str += "_Num"
						} else if after, ok := strings.CutPrefix(str, "%"); ok {
							str = after
							str += "_Len"
						}
					}
...
`
	...
	if len(args) > 0 && args[0] == "-gbf" {
		...
		for str := range unsq {
			...
			if truncate {
				...
				before, _, ok := strings.Cut(str, "FEATURES             ")
				if !ok {
					continue
				}
	...
	if len(args) > 0 && args[0] == "-txf" {
		...
		for {
			...
			case "-pattern":
				txt, ok := nextArg()
				if ok {
					// allow match with trailing spaces to unambiguously detect flatfile LOCUS line
					if txt == "LOCUS       " {
						pattern = txt
					} else {
	...
	// READ TEXT FILE, PARTITION BY PATTERN, SPLIT BY RECORD COUNT OR SIZE

	if len(args) > 0 && args[0] == "-splt" {
		...
		for {
			...
			case "-pattern":
				...
				if ok {
					pattern = txt
					// only flatfile LOCUS line allows trailing spaces
					if txt != "LOCUS       " {
						pattern = strings.TrimSpace(txt)
					}
				}
			case "-by-num", "-split-by-num", "-by-number", "-split-by-number":
				txt, ok := nextArg()
				if ok {
					val, err := strconv.Atoi(txt)
					if err == nil && val > 0 {
			...
			case "-by-len", "-split-by-len", "-by-length", "-split-by-length":
				txt, ok := nextArg()
				if ok {
					val, err := strconv.Atoi(txt)
					if err == nil && val > 0 {
		...
		if pattern == "" {
			eutils.DisplayError("No required -pattern argument in -splt")
			os.Exit(1)
		}

		if byLen == 0 && byNum == 0 {
			eutils.DisplayError("No required -by-num or -by-len in -splt")
			os.Exit(1)
		}
		...
		saveTextFile := func(inp <-chan eutils.TextRecord) <-chan string {
			...
			if out == nil {
				eutils.DisplayError("Unable to create saver channel")
				os.Exit(1)
			}

			go func(inp <-chan eutils.TextRecord, out chan<- string) {
				...
				for curr := range inp {
					...
					if byNum > 0 {
						...
						if cumulative > 0 && cumulative+len(str)+retlen >= byLen {
							fl.Close()
		...
		inp := eutils.CreateTextStreamer(in)

		txtq := eutils.CreateTextProducer(pattern, "", "", 0, 0, inp)

		splq := saveTextFile(txtq)

		if inp == nil || txtq == nil || splq == nil {
			eutils.DisplayError("Unable to create record splitter")
			os.Exit(1)
		}
