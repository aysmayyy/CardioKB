	...
	indexEmbl := func(inp io.Reader, out chan<- string) {
		...
		for {
			...
			for {
				...
				if after, ok := strings.CutPrefix(line, "AC   "); ok {
				...
				if after, ok := strings.CutPrefix(line, "SV   "); ok {
				...
				for strings.HasPrefix(line, "RN   ") {
					...
					collectRefStrings := func(pfx string) string {
						...
						for {
							...
							if after, ok := strings.CutPrefix(line, pfx); ok {
								str = after
								arry = append(arry, str)
					...
					pmid := ""
					if after, ok := strings.CutPrefix(rx, "PubMed="); ok {
						rx = after
