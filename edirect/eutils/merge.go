...
func parseInvDocument(txt string) (string, string, string, string) {
	...
	// start tag can have position attributes before closing angle bracket
	pos = strings.Index(uid, ">")
	if pos > 0 {
		uid = uid[pos+1:]
	}
	uid = strings.TrimSuffix(uid, "</"+fld+">")
	...
	go func(inp []<-chan Plex, out chan<- Plex) {
		...
		for hp.Len() > 0 {
			...
			if prevIdent == curr.Ident && prevField == curr.Field && prevTag == curr.Tag {
				...
				txt := curr.Text[:]
				if before, ok := strings.CutSuffix(txt, tailTag); ok {
					txt = before
				}
