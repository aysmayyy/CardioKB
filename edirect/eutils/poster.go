...
// during indexing.
//
// This function is typically called with 20 files at a time.
func CreatePromoters(prom, db, fields string, isLink bool, files []string) <-chan string {
	...
	xmlPromoter := func(wg *sync.WaitGroup, fileName string, out chan<- string) {
		...
		uintsToVarints := func(vals []uint32) []byte {

			var buffr []byte

			appendOneUvarint := func(val uint32) {
		...
		addOnePosting := func(term string, data []int32, atts []string) {
			...
			if alength > 0 && dlength > 0 && dlength != alength {
		...
		PartitionXML("InvDocument", "", false, rdr,
			func(str string) {
				...
				if isLink {
					...
					// do not test for hyphenated fields, e.g., UID-02 and ACCN-XP
					if currTag != tag && strings.Index(fld, "-") < 0 {
						DisplayError("InvDocument tag '%s' != currTag '%s' for id '%s' and fld '%s'", tag, currTag, id, fld)
					}
				}
...
func getPostingFutures(prom, term, field string, isLink bool) ([]Master, []byte) {
...
func getPostingTerms(prom, term, field string) []string {

	dpath, key := PostingPath(prom, field, term, false)
	if dpath == "" {
		return nil
	}
...
func getPostingIDs(indx []Master, trms []byte, prom, term, field string, simple, isLink bool) ([]int32, [][]uint32) {
	...
	if (indx == nil || len(indx) < 1) && (trms == nil || len(trms) < 1) {
		indx, trms = getPostingFutures(prom, term, field, isLink)
	}
	...
	varintsToUints := func(buf []byte) []uint32 {
...
func postingIDsFuture(base, term, field string, dist int, isLink bool) <-chan Arrays {
	...
	postingFuture := func(base, term, field string, dist int, out chan<- Arrays) {
		...
		data, ofst := getPostingIDs(nil, nil, base, term, field, false, isLink)
