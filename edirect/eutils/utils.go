...
// PrintMemory is adapted from PrintMemUsage in: https://gophercoding.com/print-current-memory-usage/
func PrintMemory() {
...
func PrintStats() {
	...
	lgc := cpuid.CPU.LogicalCores
	if lgc > 0 {
		fmt.Fprintf(os.Stderr, "Thds %d\n", lgc)
	}

	fmt.Fprintf(os.Stderr, "\nMmry %d\n", bToGb(memory.TotalMemory()))
	fmt.Fprintf(os.Stderr, "Free %d\n\n", bToGb(memory.FreeMemory()))
...
// ReadBinaryFileToBytes reads all bytes in a binary file, decompressing automatically if it has a ".gz" suffix
func ReadBinaryFileToBytes(fileName string) []byte {
