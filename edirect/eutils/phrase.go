...
func ResolveArchivePaths(db string) LclArchPathMap {
	...
	// first check for presence of edirect.ini configuration file in folder with EDirect executables,
	// allows quick local archive test without needing to set environment variable in .bash_profile
	ex, err := os.Executable()
	if err == nil {
		exPath := filepath.Dir(ex)
		fpath := filepath.Join(exPath, "edirect.ini")
		_, err = os.Stat(fpath)
		if err == nil {
			config = fpath
		}
	...
	// otherwise try configuration file specified by environment variable
	if config == "" {
	...
	if topPaths["ARCHIVE"] == "" {
		...
		// set group paths from specific environment variables
...
func printTermCount(base, term, field string) int {
	...
	data, _ := getPostingIDs(nil, nil, base, term, field, true, false)
	size := len(data)
...
func printTermCounts(base, term, field string) int {
	...
	indx, trms := getPostingFutures(base, term, field, false)
...
func printTermPositions(indx []Master, trms []byte, base, term, field string) int {

	data, ofst := getPostingIDs(indx, trms, base, term, field, false, false)
	size := len(data)
...
func evaluateQuery(base, db, phrase string, clauses []string, noStdout, isLink bool) (int, []int32) {
	...
	eval := func(str string) ([]int32, [][]uint32, int) {
		...
		if noProx && len(words) == 1 {
			...
			term = strings.Replace(term, "_", " ", -1)
			data, _ := getPostingIDs(nil, nil, base, term, field, true, isLink)
			count++
			return data, nil, 1
		}
...
func prepareExact(str, sfx string, deStop bool) string {
	...
	words := strings.FieldsSeq(phrases)
...
func ProcessCount(db, phrase string, plrl, psns, tlst, deStop bool) int {
	...
	splitIntoWords := func(str string) []string {
		...
		for segment := range parts {
			...
			words := strings.FieldsSeq(segment)
	...
	checkTermCounts := func(txt string) {
		...
		for _, term := range words {
			...
			if tlst {
				indx, trms := getPostingFutures(postingsBase, term, field, false)
				if trms == nil || len(trms) < 1 {
					continue
				}

				strs := strings.Split(string(trms), "\n")
				// strs := getPostingTerms(postingsBase, term, field)
				if strs != nil && len(strs) > 0 {
					for _, str := range strs {
						count += printTermPositions(indx, trms, postingsBase, str, field)
					}
				}
			} else if psns {
				count += printTermPositions(nil, nil, postingsBase, term, field)
			} else if plrl {
			...
			visitOneTop := func(top string) {
				...
				visitSubFolders = func(path string) {
					...
					for _, item := range contents {
						...
						if before, ok := strings.CutSuffix(name, ".trm"); ok {
							name = before
	...
	fetchTermSet := func(inp <-chan []string) <-chan []string {
		...
		fetchByGroup := func(inp <-chan []string, out chan<- []string) {
			...
			for paths := range inp {
				...
				slc := SliceToChan(paths, chanDepth)
				tms := fetchTerms(slc)
				var tlst []string
				for trm := range tms {
