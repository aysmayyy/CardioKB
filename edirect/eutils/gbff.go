		...
		for {
			...
			if strings.HasPrefix(line, "LOCUS") {
				...
				if ln == 8 {
					moleculetype := cols[4]
					strandedness := ""
					if after, ok := strings.CutPrefix(moleculetype, "ds-"); ok {
						moleculetype = after
						strandedness = "double"
					} else if after, ok := strings.CutPrefix(moleculetype, "ss-"); ok {
						moleculetype = after
						strandedness = "single"
					} else if after, ok := strings.CutPrefix(moleculetype, "ms-"); ok {
						moleculetype = after
						strandedness = "mixed"
			...
			if after, ok := strings.CutPrefix(line, "DEFINITION"); ok {
			...
			if after, ok := strings.CutPrefix(line, "ACCESSION"); ok {
			...
			if strings.HasPrefix(line, "VERSION") {
				...
				if len(cols) == 2 {
					...
					if after, ok := strings.CutPrefix(cols[2], "GI:"); ok {
						gi = after
					}
			...
			if after, ok := strings.CutPrefix(line, "DBLINK"); ok {
			...
			if after, ok := strings.CutPrefix(line, "DBSOURCE"); ok {
			...
			if after, ok := strings.CutPrefix(line, "  ORGANISM"); ok {
				...
				line = nextLine()
				row++
				if after, ok := strings.CutPrefix(line, twelvespaces); ok {
					txt := after
			...
			for {
				...
				if after, ok := strings.CutPrefix(line, "  AUTHORS"); ok {

					txt := after
					auths := readContinuationLines(txt)

					rec.WriteString("        <INSDReference_authors>\n")
					authors := strings.SplitSeq(auths, ", ")
					for auth := range authors {
						auth = strings.TrimSpace(auth)
						if auth == "" {
							continue
						}
						pair := strings.SplitSeq(auth, " and ")
						for name := range pair {
				...
				if after, ok := strings.CutPrefix(line, "  CONSRTM"); ok {
				...
				if after, ok := strings.CutPrefix(line, "  JOURNAL"); ok {
				...
				if after, ok := strings.CutPrefix(line, "   PUBMED"); ok {
				...
				if after, ok := strings.CutPrefix(line, "  MEDLINE"); ok {
				...
				if after, ok := strings.CutPrefix(line, "  REMARK"); ok {
			...
			if after, ok := strings.CutPrefix(line, "PRIMARY"); ok {
				...
				for {
					...
					parseloc = func(str string) []string {
						...
						if strings.HasPrefix(str, "join(") && strings.HasSuffix(str, ")") {
							...
							items := strings.SplitSeq(str, ",")

							for thisloc := range items {
							...
							for _, thisloc := range items {
								...
								if ln == 2 {
									...
									lf := ""
									rt := ""
									if after, ok := strings.CutPrefix(fst, "<"); ok {
										fst = after
										rt = ">"
									}
									if after, ok := strings.CutPrefix(scd, ">"); ok {
										scd = after
										lf = "<"
									}
					...
					for _, thisloc := range items {
						...
						if len(pts) == 2 {
							...
							fr := pts[0]
							to := pts[1]
							if after, ok := strings.CutPrefix(fr, "<"); ok {
								fr = after
								prime5 = true
							}
							if after, ok := strings.CutPrefix(to, ">"); ok {
								to = after
								prime3 = true
							}
							...
							if len(crt) == 2 {
								...
								// pt
								pt := pts[0]
								if after, ok := strings.CutPrefix(pt, "<"); ok {
									pt = after
									prime5 = true
								}
								if after, ok := strings.CutPrefix(pt, ">"); ok {
			...
			if after, ok := strings.CutPrefix(line, "BASE COUNT"); ok {
