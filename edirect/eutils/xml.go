	...
	nextToken := func(idx int) (int, int, string, string, int) {
		...
		if plainContent && ch == '<' {
			...
			if inFirst[ch] {
				...
				idx++
				if idx >= txtlen {
					DisplayError("Unexpected inSlash end of text, line %d, text '%s'", currentLineCount(idx-1), text[:])
				...
				if inFirst[ch] {
					idx++
					if idx >= txtlen {
						DisplayError("Unexpected inFirst end of text, line %d, text '%s'", currentLineCount(idx-1), text[:])
						return NOTAG, NONE, "", "", idx
					}
					ch = text[idx]
					for inElement[ch] {
						idx++
						if idx >= txtlen {
							DisplayError("Unexpected inElement end of text, line %d, text '%s'", currentLineCount(idx-1), text[:])
							return NOTAG, NONE, "", "", idx
						}
						ch = text[idx]
					}
					...
					if ch != '>' {
						...
						for inBlank[ch] {
							idx++
							if idx >= txtlen {
								DisplayError("Unexpected inBlank end of text, line %d, text '%s'", currentLineCount(idx-1), text[:])
								return NOTAG, NONE, "", "", idx
							}
							ch = text[idx]
						}
